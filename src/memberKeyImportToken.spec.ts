...
import {
  AWALA_PDA,
  MEMBER_MONGO_ID, MEMBER_PUBLIC_KEY_MONGO_ID,
  TEST_SERVICE_OID,
...
import { BUNDLE_REQUEST_TYPE, MemberBundleRequestPayload } from './events/bundleRequest.event.js';
...
const { createMemberKeyImportToken, processMemberKeyImportToken } = await import('./memberKeyImportToken.js');
...
describe('member key import token', () => {
  ...
  describe('processMemberKeyImportToken', () => {
    const mockEventEmitter = mockEmitter();
    ...
    test('Valid data should return be processed', async () => {
      ...
      expect(mockCreateMemberPublicKey).toHaveBeenCalledOnceWith(MEMBER_MONGO_ID, {
        publicKey: publicKeyBase64,
        serviceOid: TEST_SERVICE_OID,
      }, serviceOptions)
      ...
      expect(mockEventEmitter.getEvents()).toContainEqual(
    ...
    test('Emitter error should not delete key import token', async () => {
      mockEventEmitter.getEmitter().setShouldThrowError(true);
      const keyImportToken = await memberKeyImportTokenModel.create({
        memberId: MEMBER_MONGO_ID,
        serviceOid: TEST_SERVICE_OID,
      });
      ...
      const error = await getPromiseRejection(
        async () => processMemberKeyImportToken({
          publicKey: publicKeyBase64,
          publicKeyImportToken: keyImportToken._id.toString(),
          awalaPda: AWALA_PDA
        }, serviceOptions),
      ...
      expect(error).toHaveProperty('message', 'Error while processing event');
