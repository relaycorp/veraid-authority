...
export async function createMemberBundleRequest(
  ...
  if (!publicKey) {
    options.logger.info({ memberPublicKeyId: requestData.publicKeyId }, 'Member public key not found');
    return {
      didSucceed: false,
    };
  }
  ...
  options.logger.info({ memberPublicKeyId: requestData.publicKeyId }, 'Member bundle request created');
